                    ...
                    <form action="{{ route('pelanggan.kontak.kirim') }}" method="POST">
                        ...
                        <div class="mb-3">
                            <input type="text" name="nama"
                                value="{{ Auth::check() ? Auth::user()->name : old('nama') }}" class="form-control"
                                placeholder="Nama Lengkap" required {{ Auth::check() ? 'readonly' : '' }}>
                        </div>
                        <div class="mb-3">
                            <input type="email" name="email"
                                value="{{ Auth::check() ? Auth::user()->email : old('email') }}" class="form-control"
                                placeholder="Alamat Email" required {{ Auth::check() ? 'readonly' : '' }}>
                        </div>
                        <div class="mb-3">
