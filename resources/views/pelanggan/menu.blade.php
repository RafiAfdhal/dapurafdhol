...
@section('content')
    ...
    {{-- Hero Section --}}
    <section class="menu-hero py-5 mb-4 bg-light text-center">
        <div class="container">
            <h1 class="section-title mb-4 display-5 fw-bold text-warning">
    ...
    <section class="container py-4">
        <div class="menu-content-wrapper d-flex flex-column flex-md-row gap-4">

            {{-- Filter Kategori --}}
            <div class="menu-filter-column mb-4 bg-white shadow-sm rounded p-3" style="min-width: 220px;">
                <h5 class="mb-3 fw-bold text-warning text-center text-md-start">Filter Kategori</h5>
            ...
            {{-- Menu Items --}}
            <div class="menu-items-column flex-grow-1">
                <div class="row row-cols-1 row-cols-sm-2 row-cols-md-2 row-cols-lg-3 g-4" id="menu-items-container">
                    @forelse ($menus as $menu)
                        <div class="col menu-item">
                            <div class="menu-card h-100 shadow-sm rounded overflow-hidden bg-white">
                                {{-- Gambar --}}
                                @if ($menu->gambar)
                                    <img src="{{ asset('storage/' . $menu->gambar) }}" class="card-img-top"
                                        alt="{{ $menu->nama }}" style="height: 180px; object-fit: cover;">
                                @else
                                ...
                                <div class="card-body d-flex flex-column p-3">
                                    ...
                                    <a href="{{ route('pelanggan.ulasan', $menu->id) }}"
                                        ...
                                        <div class="d-flex rating-stars-display fs-6 text-warning me-2">
                                            @php
                                        ...
                                        <small class="text-muted">({{ $menu->reviews->count() }} ulasan)</small>
                                    </a>

                                    {{-- Harga & Tombol --}}
                                    <div class="d-flex justify-content-between align-items-center mt-auto">
                                        <div>
                                            <span class="item-price fw-bold text-dark d-block">
                                        ...
                                        <div class="d-flex align-items-center">
                                            @if ($menu->stok > 0)
                                                ...
                                                <form action="{{ route('pelanggan.cart.add', $menu->id) }}" method="POST"
                                                    ...
                                                    @csrf
                                                    <button type="submit"
                                                        class="btn btn-outline-warning btn-sm rounded-circle ms-2">
